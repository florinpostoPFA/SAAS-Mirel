...
async function runTests() {
  ...
  for (const test of tests) {
    ...
    try {
      ...
      // Check blocking
      if (test.expected.shouldBlock) {
        const isQuestion =
          response?.data?.type === "question" ||
          responseText.includes("?");
        if (!isQuestion) {
          details.push(`  ❌ Expected blocking question but proceeded`);
          testPassed = false;
        } else if (
          test.expected.askQuestion &&
          !responseText.toLowerCase().includes(
            test.expected.askQuestion.toLowerCase()
          )
        ) {
          details.push(
            `  ❌ Blocking worked but question text missing: "${test.expected.askQuestion}"`
          );
          testPassed = false;
        } else {
          details.push(`  ✅ Blocking + question confirmed`);
        }
      }

      // Check "should not contain"
      if (test.expected.shouldNotContain) {
        const hasUnwanted = test.expected.shouldNotContain.some((p) =>
          responseText.toLowerCase().includes(p.toLowerCase())
        );
        if (hasUnwanted) {
          details.push(
            `  ❌ Response contains unwanted text: ${test.expected.shouldNotContain.join(", ")}`
          );
          testPassed = false;
        } else {
          details.push(`  ✅ Unwanted text cleared`);
        }
      }
